...
function changeStats(timeframeID) {
  ...
  .done((data) => {
    $('#revenue').text(data.timeframeStats.revenue.toFixed(2) + ' ' + userCurrency)
    $('#fees').text(data.timeframeStats.fees.toFixed(2) + ' ' + userCurrency)
    $('#profit').text(data.timeframeStats.profit.toFixed(2) + ' ' + userCurrency)
    $('#return').text(data.timeframeStats.str_return.toFixed(2) + ' %')
    $('#max-win').text(data.timeframeStats.max.toFixed(2) + ' ' + userCurrency)
    $('#max-loss').text(data.timeframeStats.min.toFixed(2) + ' ' + userCurrency)
    $('#avg-win').text(data.timeframeStats.avg_win.toFixed(2) + ' ' + userCurrency)
    $('#avg-loss').text(data.timeframeStats.avg_loss.toFixed(2) + ' ' + userCurrency)
    $('#avg').text(data.timeframeStats.avg.toFixed(2) + ' ' + userCurrency)
    $('#avg-hold').text(data.timeframeStats.avg_holding.toFixed(2) + ' days')
    $('#gross-loss').text(data.timeframeStats.gross_loss.toFixed(2) + ' ' + userCurrency)
    $('#profit-factor').text(data.timeframeStats.profit_factor.toFixed(2))
    $('#playoff-ratio').text(data.timeframeStats.playoff.toFixed(2))
    $('#max-drawdown').text(data.timeframeDrawdown.max_drawdown.toFixed(2) + ' ' + userCurrency)
